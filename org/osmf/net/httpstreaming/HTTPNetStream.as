package org.osmf.net.httpstreaming
{
   ...
   public class HTTPNetStream extends NetStream
   {
      ...
      private var prevRequest:HTTPStreamRequest;
      
      private var _failRetryCount:Number = 0;
      
      public var _dataBytesLoaded:Number = 0;
      
      public var _dataBytesTotal:Number = 0;
      
      public var _downDataBytes:Number = 0;
      
      private var _preSegmentDuration:Array;
      
      private var _seekSegID:int = 0;
      
      private var _bSeekSeg:Boolean = false;
      ...
      override public function seek(param1:Number) : void
      {
         ...
         if(this._state != HTTPStreamingState.INIT)
         {
            ...
            this._seekSegID = this._seekTarget * this.indexHandler.getTotalFragments() / this.indexHandler.getTotalTime();
            this._seekTime = -1;
            this._bSeekSeg = true;
      ...
      {
         var _loc2_:int = 0;
         var _loc3_:FLVTagScriptDataObject = null;
         var _loc4_:ByteArray = null;
         var _loc5_:Number = NaN;
         var _loc6_:FLVTagVideo = null;
         var _loc7_:FLVTagVideo = null;
         var _loc8_:FLVTagVideo = null;
         var _loc9_:int = 0;
         var _loc10_:int = 0;
         var _loc11_:FLVTagAudio = null;
         if(this._insertScriptDataTags)
         {
            _loc2_ = 0;
            while(_loc2_ < this._insertScriptDataTags.length)
            {
               ...
               _loc3_.write(_loc4_);
               this._flvParserProcessed = this._flvParserProcessed + _loc4_.length;
         ...
         if(this._playForDuration >= 0)
         {
            if(this._initialTime >= 0)
            {
               _loc5_ = param1.timestamp / 1000 + this._fileTimeAdjustment;
               if(_loc5_ > this._initialTime + this._playForDuration)
               {
         ...
         if(this._enhancedSeekTarget < 0)
         {
            ...
            if(this._playForDuration >= 0)
            {
               if(this._segmentDuration >= 0 && this._flvParserIsSegmentStart)
               {
                  ...
                  _loc5_ = param1.timestamp / 1000 + this._fileTimeAdjustment;
                  if(_loc5_ + this._segmentDuration >= this._initialTime + this._playForDuration)
                  {
                     return true;
                  }
                  this._flvParserDone = true;
         ...
         if(param1 is FLVTagVideo)
         {
            ...
            {
               _loc6_ = new FLVTagVideo();
               _loc6_.timestamp = param1.timestamp;
            ...
            if(param1.timestamp / 1000 + this._fileTimeAdjustment >= this._enhancedSeekTarget)
            {
               ...
               _loc7_ = new FLVTagVideo();
               _loc7_.timestamp = param1.timestamp;
               ...
               {
                  _loc8_ = this._enhancedSeekTags[_loc2_];
                  if(_loc8_.codecID == FLVTagVideo.CODEC_ID_AVC && _loc8_.avcPacketType == FLVTagVideo.AVC_PACKET_TYPE_NALU)
                  {
         ...
         {
            _loc11_ = param1 as FLVTagAudio;
            if(_loc11_.isCodecConfiguration)
            {
               _loc4_ = new ByteArray();
               param1.write(_loc4_);
               this._flvParserProcessed = this._flvParserProcessed + _loc4_.length;
         ...
         if(!this._manualSwitchMode)
         {
            ...
            if(this._lastDownloadRatio < 1)
            {
               if(this.qualityLevel > 0)
               {
                  ...
                  _loc3_ = this._qualityRates[_loc2_] / this._qualityRates[this.qualityLevel];
      ...
      private function processAndAppend(param1:ByteArray) : uint
      {
         ...
         if(this._flvParser)
         {
            ...
            _loc3_ = _loc3_ + this._flvParserProcessed;
      ...
      {
		 // ExternalInterface.call("msg","[HTTPNetStream].[onMainTimer]");
         var _loc2_:ByteArray = null;
         var _loc3_:HTTPStreamRequest = null;
         var _loc4_:Date = null;
         var _loc5_:Boolean = false;
         var _loc6_:Object = null;
         var _loc7_:FLVTagScriptDataObject = null;
         var _loc8_:ByteArray = null;
         var _loc9_:Object = null;
         var _loc10_:FLVTagScriptDataObject = null;
         var _loc11_:int = 0;
         var _loc12_:int = 0;
         var _loc13_:IDataInput = null;
         switch(this._state)
         {
            ...
            case HTTPStreamingState.LOAD_WAIT:
               ...
               {
                  if(this.bufferLength < Math.max(7.5,60))
                  {
                     this.setState(HTTPStreamingState.LOAD_NEXT);
                  }
               }
               else if(this.bufferLength < Math.max(3.75,60))
               {
                  this.setState(HTTPStreamingState.LOAD_NEXT);
               }
               break;
            case HTTPStreamingState.LOAD_NEXT:
               ...
               if(this.qualityLevelHasChanged)
               {
                  ...
                  _loc9_ = new Object();
                  _loc9_.code = NetStreamCodes.NETSTREAM_PLAY_TRANSITION_COMPLETE;
                  _loc9_.level = "status";
                  _loc10_ = new FLVTagScriptDataObject();
                  _loc10_.objects = ["onPlayStatus",_loc9_];
            ...
            case HTTPStreamingState.LOAD_SEEK:
               ...
               this.setState(HTTPStreamingState.LOAD);
               break;
            case HTTPStreamingState.RETRY:
               this._failRetryCount++;
               this._segmentDuration = -1;
               if(this.prevRequest != null && this.prevRequest.urlRequest != null)
               {
                  this._loadComplete = false;
                  this._urlStreamVideo.load(this.prevRequest.urlRequest);
                  _loc4_ = new Date();
               ...
               else if(this.prevRequest != null && this.prevRequest.retryAfter >= 0)
               {
                  _loc4_ = new Date();
                  this._retryAfterWaitUntil = _loc4_.getTime() + 1000 * this.prevRequest.retryAfter;
                  switch(this._prevState)
                  {
                     case HTTPStreamingState.LOAD_SEEK:
                     case HTTPStreamingState.LOAD_SEEK_RETRY_WAIT:
               ...
               else
               {
                  ...
                  this.processAndAppend(_loc2_);
                  this.setState(HTTPStreamingState.STOP);
                  if(this.prevRequest != null && this.prevRequest.unpublishNotify)
                  {
            ...
            case HTTPStreamingState.LOAD:
               ...
               else if(_loc3_ != null && _loc3_.retryAfter >= 0)
               {
                  _loc4_ = new Date();
            ...
            case HTTPStreamingState.LOAD_NEXT_RETRY_WAIT:
               _loc4_ = new Date();
               if(_loc4_.getTime() > this._retryAfterWaitUntil)
               {
                  this.prevRequest = this.indexHandler.getPrevFile(this.qualityLevel);
                  this.setState(HTTPStreamingState.RETRY);
               }
               break;
            case HTTPStreamingState.PLAY_START_NEXT:
            ...
            case HTTPStreamingState.PLAY:
               ...
               if(this._dataAvailable)
               {
                  ...
                  _loc13_ = null;
                  this._dataAvailable = false;
                  while(this._state == HTTPStreamingState.PLAY && (_loc13_ = this.byteSource(this._urlStreamVideo,this.fileHandler.inputBytesNeeded)))
                  {
                     _loc2_ = this.fileHandler.processFileSegment(_loc13_);
                     _loc12_ = _loc12_ + this.processAndAppend(_loc2_);
            ...
            case HTTPStreamingState.STOP:
               _loc6_ = new Object();
               _loc6_.code = NetStreamCodes.NETSTREAM_PLAY_COMPLETE;
               _loc6_.level = "status";
               _loc7_ = new FLVTagScriptDataObject();
               _loc7_.objects = ["onPlayStatus",_loc6_];
      ...
      private function onURLStatus(param1:ProgressEvent) : void
      {
         ...
         var _loc3_:Number = NaN;
         this._dataAvailable = true;
         _loc2_ = Math.floor(this._segmentDuration * param1.bytesLoaded / param1.bytesTotal);
         if(this._bSeekSeg)
         {
            if(this._preSegmentDuration[this._seekSegID] > 0)
            {
               this._dataBytesLoaded = this._preSegmentDuration[this._seekSegID] * (this._seekSegID + 1);
            }
            else
            {
               this._dataBytesLoaded = this._seekTarget + _loc2_;
            }
            this._dataBytesTotal = this.indexHandler.getTotalTime();
            this._downDataBytes = param1.bytesLoaded;
         }
         else
         {
            _loc3_ = 0;
            if(this.indexHandler.getCurrFragID() > 0)
            {
               _loc3_ = this._preSegmentDuration[this.indexHandler.getCurrFragID() - 1] * this.indexHandler.getCurrFragID();
            }
            _loc2_ = Math.floor(this._segmentDuration * param1.bytesLoaded / param1.bytesTotal);
            this._dataBytesLoaded = _loc3_ + _loc2_;
            this._dataBytesTotal = this.indexHandler.getTotalTime();
            this._downDataBytes = param1.bytesLoaded;
         }
      }
      ...
      private function onURLComplete(param1:Event) : void
      {
         ...
         this._bSeekSeg = false;
         this._preSegmentDuration[this.indexHandler.getCurrFragID()] = this._segmentDuration;
      }
      ...
      private function onRequestLoadIndexFile(param1:HTTPStreamingIndexHandlerEvent) : void
      {
         ...
         urlLoader = new URLLoader(event.request);
      ...
      private function onIndexReady(param1:HTTPStreamingIndexHandlerEvent) : void
      {
         ...
         if(!this.indexIsReady)
         {
            ...
            this._preSegmentDuration = new Array(this.indexHandler.getTotalFragments());
            _loc2_ = 0;
            while(_loc2_ < this._preSegmentDuration.length)
            {
               this._preSegmentDuration[_loc2_] = 0;
               _loc2_++;
            }
         }
      }
      ...
      {
         if(this._failRetryCount < 6)
         {
            this._retryAfterWaitUntil = this._lastDownloadStartTime + 1000 * this.prevRequest.retryAfter;
            this.setState(HTTPStreamingState.LOAD_NEXT_RETRY_WAIT);
         }
         else
         {
      ...
      private function handleURLError() : void
      {
         dispatchEvent(new NetStatusEvent(NetStatusEvent.NET_STATUS,false,false,{
            ...
            "description":"获取服务器数据失败，请检查网络后重新刷新页面",
            "level":"error"
         }));
      }
