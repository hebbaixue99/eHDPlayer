package org.osmf.net.httpstreaming.f4f
{
   ...
   public class HTTPStreamingF4FFileHandler extends HTTPStreamingFileHandlerBase
   {
      ...
      {
		  ExternalInterface.call("msg","[HTTPStreamingF4F].[HTTPStreamingF4FFileHandler()]");
      ...
      override public function beginProcessFile(param1:Boolean, param2:Number) : void
      {
         ...
         this._seekToTime = !!param1?Number(param2):Number(0);
      ...
      override public function processFileSegment(param1:IDataInput) : ByteArray
      {
         ...
         var _loc3_:Number = F4FConstants.FIELD_SIZE_LENGTH + F4FConstants.FIELD_TYPE_LENGTH;
         if(this._boxInfoPending)
         {
            ...
            {
               this._bytesReadSinceAfraStart = this._bytesReadSinceAfraStart + _loc3_;
            }
            ...
            {
               _loc3_ = _loc3_ + F4FConstants.FIELD_LARGE_SIZE_LENGTH;
               this._ba.position = 0;
         ...
         else if(this._bytesNeeded > 0)
         {
            ...
            if(this._countingReadBytes)
            {
               this._bytesReadSinceAfraStart = this._bytesReadSinceAfraStart + (this._nextBox.size - _loc3_);
            }
            this._ba.position = _loc4_;
            if(this._nextBox.type == F4FConstants.BOX_TYPE_ABST)
            {
               _loc5_ = this._parser.readAdobeBootstrapBox(this._nextBox);
               if(_loc5_ != null)
               {
      ...
      private function getMDATBytes(param1:IDataInput, param2:Boolean) : ByteArray
      {
         ...
         {
            _loc4_ = this._mdatBytesPending < param1.bytesAvailable?uint(this._mdatBytesPending):uint(param1.bytesAvailable);
            if(!param2 && _loc4_ > MAX_BYTES_PER_MDAT_READ)
            {
               _loc4_ = MAX_BYTES_PER_MDAT_READ;
            }
            _loc3_ = new ByteArray();
            this._mdatBytesPending = this._mdatBytesPending - _loc4_;
      ...
      private function skipSeekBytes(param1:IDataInput) : void
      {
         ...
         if(this._bytesReadSinceAfraStart < this._mdatBytesOffset)
         {
            ...
            this._bytesReadSinceAfraStart = this._bytesReadSinceAfraStart + _loc2_;
            this._mdatBytesPending = this._mdatBytesPending - _loc2_;
         }
      }
      ...
      {
		  ExternalInterface.call("msg","[HTTPStreamingF4F].[this._afra.timeScale]="+this._afra.timeScale);
