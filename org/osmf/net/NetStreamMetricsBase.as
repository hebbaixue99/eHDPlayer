package org.osmf.net
{
   ...
   public class NetStreamMetricsBase extends EventDispatcher
   {
      ...
      public function set resource(param1:DynamicStreamingResource) : void
      {
         ...
         this._maxAllowedIndex = param1 != null?int(param1.streamItems.length - 1):0;
      }
      ...
      protected function calculateMetrics() : void
      {
         ...
         try
         {
            this._maxFPS = this.netStream.currentFPS > this._maxFPS?Number(this.netStream.currentFPS):Number(this._maxFPS);
            ...
            this._averageDroppedFPS = this._averageDroppedFPSArray.length < DEFAULT_AVG_FRAMERATE_SAMPLE_SIZE?Number(0):Number(totalDroppedFrameRate / this._averageDroppedFPSArray.length);
            return;
         }
         catch(error:Error)
         {
