package org.osmf.metadata
{
   ...
   public class TimelineMetadata extends Metadata
   {
      ...
      {
         return !!this.temporalValueCollection?int(this.temporalValueCollection.length):0;
      }
      ...
      override public function addValue(param1:String, param2:Object) : void
      {
         ...
         {
            _loc5_ = this.findTemporalMetadata(0,this.temporalValueCollection.length - 1,_loc3_);
            if(_loc5_ < 0)
            {
               _loc5_ = _loc5_ * -1;
      ...
      override public function removeValue(param1:String) : *
      {
         ...
         var _loc3_:* = null;
         var _loc4_:int = !!this.temporalValueCollection?int(this.findTemporalMetadata(0,this.temporalValueCollection.length - 1,_loc2_)):-1;
         if(_loc4_ >= 0)
         {
            this.temporalKeyCollection.splice(_loc4_,1);
            _loc3_ = this.temporalValueCollection.splice(_loc4_,1)[0];
      ...
      override public function getValue(param1:String) : *
      {
         ...
         if(!isNaN(_loc2_))
         {
            ...
            {
               _loc4_ = this.temporalKeyCollection[_loc3_];
               if(_loc4_ == _loc2_)
               {
      ...
      private function checkTemporalMetadata(param1:int, param2:Number) : Boolean
      {
         ...
         if(this.temporalValueCollection[param1].time >= param2 - TOLERANCE && this.temporalValueCollection[param1].time <= param2 + TOLERANCE && param1 != this.lastFiredTemporalMetadataIndex)
         {
            ...
            _loc5_ = this.calcNextTime(param1);
            _loc6_ = (_loc5_ - _loc4_) * 1000 / 4;
            _loc6_ = _loc6_ > CHECK_INTERVAL?Number(_loc6_):Number(CHECK_INTERVAL);
            if(_loc4_ == _loc5_)
            {
               this.startTimer(false);
