package org.osmf.media
{
   ...
   public class MediaPlayer extends TraitEventDispatcher
   {
      ...
      {
         return !!this.canPlay?Boolean((this.getTraitOrThrow(MediaTraitType.PLAY) as PlayTrait).canPause):false;
      }
      ...
      {
         return !!this.hasAudio?Number(AudioTrait(this.getTraitOrThrow(MediaTraitType.AUDIO)).volume):Number(this.mediaPlayerVolume);
      }
      ...
      {
         return !!this.hasAudio?Boolean(AudioTrait(this.getTraitOrThrow(MediaTraitType.AUDIO)).muted):Boolean(this.mediaPlayerMuted);
      }
      ...
      {
         return !!this.hasAudio?Number(AudioTrait(this.getTraitOrThrow(MediaTraitType.AUDIO)).pan):Number(this.mediaPlayerAudioPan);
      }
      ...
      {
         return !!this.canSeek?Boolean((this.getTraitOrThrow(MediaTraitType.SEEK) as SeekTrait).seeking):false;
      }
      ...
      {
         return !!this._hasDisplayObject?Number((this.getTraitOrThrow(MediaTraitType.DISPLAY_OBJECT) as DisplayObjectTrait).mediaWidth):Number(NaN);
      }
      
      public function get mediaHeight() : Number
      {
         return !!this._hasDisplayObject?Number((this.getTraitOrThrow(MediaTraitType.DISPLAY_OBJECT) as DisplayObjectTrait).mediaHeight):Number(NaN);
      }
      
      public function get autoDynamicStreamSwitch() : Boolean
      {
         return !!this.isDynamicStream?Boolean((this.getTraitOrThrow(MediaTraitType.DYNAMIC_STREAM) as DynamicStreamTrait).autoSwitch):Boolean(this.mediaPlayerAutoDynamicStreamSwitch);
      }
      ...
      {
         return !!this.isDynamicStream?int((this.getTraitOrThrow(MediaTraitType.DYNAMIC_STREAM) as DynamicStreamTrait).currentIndex):0;
      }
      
      public function get numDynamicStreams() : int
      {
         return !!this.isDynamicStream?int((this.getTraitOrThrow(MediaTraitType.DYNAMIC_STREAM) as DynamicStreamTrait).numDynamicStreams):0;
      }
      ...
      {
         return !!this.isDynamicStream?int((this.getTraitOrThrow(MediaTraitType.DYNAMIC_STREAM) as DynamicStreamTrait).maxAllowedIndex):int(this.mediaPlayerMaxAllowedDynamicStreamIndex);
      }
      ...
      {
         return !!this.isDynamicStream?Boolean((this.getTraitOrThrow(MediaTraitType.DYNAMIC_STREAM) as DynamicStreamTrait).switching):false;
      }
      ...
      {
         return !!this.temporal?Number((this.getTraitOrThrow(MediaTraitType.TIME) as TimeTrait).duration):Number(0);
      }
      
      public function get currentTime() : Number
      {
         return !!this.temporal?Number((this.getTraitOrThrow(MediaTraitType.TIME) as TimeTrait).currentTime):Number(0);
      }
      
      public function get buffering() : Boolean
      {
         return !!this.canBuffer?Boolean((this.getTraitOrThrow(MediaTraitType.BUFFER) as BufferTrait).buffering):false;
      }
      
      public function get bufferLength() : Number
      {
         return !!this.canBuffer?Number((this.getTraitOrThrow(MediaTraitType.BUFFER) as BufferTrait).bufferLength):Number(0);
      }
      
      public function get bufferTime() : Number
      {
         return !!this.canBuffer?Number((this.getTraitOrThrow(MediaTraitType.BUFFER) as BufferTrait).bufferTime):Number(this.mediaPlayerBufferTime);
      }
      ...
      {
         return !!this.hasDRM?Number(DRMTrait(media.getTrait(MediaTraitType.DRM)).period):Number(NaN);
      }
      ...
      {
         var _loc4_:TimeTrait = null;
         var _loc5_:PlayTrait = null;
         var _loc6_:AudioTrait = null;
         var _loc7_:DynamicStreamTrait = null;
         var _loc8_:DisplayObjectTrait = null;
         var _loc9_:LoadTrait = null;
         var _loc10_:BufferTrait = null;
		 ...
		 ExternalInterface.call("msg","[MediaPlayer].[updateTraitListeners]");
         switch(param1)
         {
            ...
            case MediaTraitType.TIME:
               ...
               _loc4_ = TimeTrait(media.getTrait(MediaTraitType.TIME));
               if(_loc4_.currentTime != 0 && this._currentTimeUpdateInterval > 0 && !isNaN(this._currentTimeUpdateInterval))
               {
            ...
            case MediaTraitType.AUDIO:
               ...
               else if(this.mediaPlayerAudioPan != _loc6_.pan)
               {
            ...
            case MediaTraitType.DISPLAY_OBJECT:
               ...
               _loc8_ = DisplayObjectTrait(media.getTrait(MediaTraitType.DISPLAY_OBJECT));
            ...
            case MediaTraitType.LOAD:
               ...
               _loc9_ = LoadTrait(media.getTrait(MediaTraitType.LOAD));
               if(_loc9_.bytesLoaded > 0)
               {
               ...
               if(param2)
               {
                  _loc11_ = (media.getTrait(param1) as LoadTrait).loadState;
                  if(_loc11_ != LoadState.READY && _loc11_ != LoadState.LOADING)
                  {
                     this.load();
                  }
      ...
      private function updateCapabilityForTrait(param1:String, param2:Boolean) : void
      {
         ...
         {
            case MediaTraitType.AUDIO:
               _loc3_ = MediaPlayerCapabilityChangeEvent.HAS_AUDIO_CHANGE;
               this._hasAudio = param2;
               break;
            case MediaTraitType.BUFFER:
               _loc3_ = MediaPlayerCapabilityChangeEvent.CAN_BUFFER_CHANGE;
               this._canBuffer = param2;
               break;
            case MediaTraitType.DISPLAY_OBJECT:
               _loc3_ = MediaPlayerCapabilityChangeEvent.HAS_DISPLAY_OBJECT_CHANGE;
               break;
            case MediaTraitType.DRM:
               _loc3_ = MediaPlayerCapabilityChangeEvent.HAS_DRM_CHANGE;
               this._hasDRM = param2;
               break;
            case MediaTraitType.DYNAMIC_STREAM:
               _loc3_ = MediaPlayerCapabilityChangeEvent.IS_DYNAMIC_STREAM_CHANGE;
               this._isDynamicStream = param2;
               break;
            case MediaTraitType.LOAD:
               _loc3_ = MediaPlayerCapabilityChangeEvent.CAN_LOAD_CHANGE;
               this._canLoad = param2;
               break;
            case MediaTraitType.PLAY:
               _loc3_ = MediaPlayerCapabilityChangeEvent.CAN_PLAY_CHANGE;
               this._canPlay = param2;
               break;
            case MediaTraitType.SEEK:
               _loc3_ = MediaPlayerCapabilityChangeEvent.CAN_SEEK_CHANGE;
               this._canSeek = param2;
               break;
            case MediaTraitType.TIME:
               _loc3_ = MediaPlayerCapabilityChangeEvent.TEMPORAL_CHANGE;
               this._temporal = param2;
         }
         if(_loc3_ != null)
         {
      ...
      private function onSeeking(param1:SeekEvent) : void
      {
         ...
         {
            this.setState(MediaPlayerState.SEEKING);
         }
